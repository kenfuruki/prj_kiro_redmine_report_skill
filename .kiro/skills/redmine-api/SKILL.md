...
## 【初回セットアップ】テストスイートのカスタムフィールド設定

> **この手順は初回のみ実施。設定完了後、このセクションは削除してください。**

テストフェーズ（IT/ST/UAT）の工程別進捗テーブルでは、テストスイートトラッカーのカスタムフィールドから予定ケース数・実績ケース数を集計します。
現在は仮のフィールド名が設定されているため、実環境に合わせて変更が必要です。

### 手順

1. Redmineに接続できる状態で、テストスイートトラッカーのチケットを1件取得する:
```bash
python scripts/redmine_tool.py issues --project {プロジェクトID} --status "*"
```
テストスイートのチケットIDを確認し、詳細を取得:
```bash
python scripts/redmine_tool.py issue {チケットID}
```

2. 出力される `custom_fields` の中から、予定ケース数・実績ケース数に該当するフィールド名を確認する

3. `scripts/dashboard_report.py` の以下の定数を実際のフィールド名に変更する:
```python
CF_PLANNED_CASES = "予定ケース数"    # ← 実際のフィールド名に変更
CF_ACTUAL_CASES = "実績ケース数"     # ← 実際のフィールド名に変更
```
...
5. **設定完了後、このセクション（「【初回セットアップ】テストスイートのカスタムフィールド設定」）を削除してください。**
